...
public class BarState {
    ...
    public void tick() {
        ...
        if (lastHealth < 0.1) reset();
        else if (lastHealth != health) handleHealthChange();
        else if (lastDmgDelay == 0.0F) reset();
    ...
    private void incrementTimers() {
        if (this.lastDmgDelay > 0) this.lastDmgDelay--;
        if (this.previousHealthDelay > 0) this.previousHealthDelay--;
    }
    ...
    private void updateAnimations() {
        if (previousHealthDelay > 0) {
            ...
            if (diff > 0) animationSpeed = diff / 10f;
